...
interface LotDetailsTabOffersProps {
    offers: OfferDto[] | undefined
    lot: LotDto | undefined
}
export function LotDetailsTabOffers({ offers, lot }: LotDetailsTabOffersProps) {
    ...
    return (
        !offers?.length ?
            <div className="flex flex-col gap-2  min-h-[20rem] bg-[url('/07b56b49-ba41-4fa7-ad9b-9599d76446eb.png')]  bg-no-repeat bg-right-bottom">
                <p>Аукцион был размещен {moment.locale('ru') && moment(lot?.created_at).fromNow()}</p>
                <p>Ждите предложения - они скоро появятся!</p>
            </div>
            :
            <>
